...
interface ClientMockListProps {
  ...
  readonly bundles: MockBundle[];
  readonly purchasedBundleIds: string[];
}

export default function ClientMockList({ mocks, userId, purchasedMockIds, bundles, purchasedBundleIds }: ClientMockListProps) {
  ...
  // Categorize mocks
  const freeMocks = mocks.filter(mock => mock.price === 0);
  const purchasedPaidMocks = mocks.filter(mock => mock.price > 0 && purchased.has(mock.id));
  const paidMocks = mocks.filter(mock => mock.price > 0 && !purchased.has(mock.id));
  ...
  // Helper function to render badge for mock status
  const renderStatusBadge = (mock: Mock) => {
  ...
  // Render a single mock card
  const renderMockCard = (mock: Mock) => {
    const isPurchased = purchased.has(mock.id);
    const isFree = mock.price === 0;
  ...
  return (
    <div className="space-y-12">
      {/* Section 1: Free and Already Purchased Mocks */}
      {(freeMocks.length > 0 || purchasedPaidMocks.length > 0) && (
        <section>
          <div className="mb-8">
            ...
            <p className="text-center text-muted-foreground">
              Free tests and your purchased mock tests - Start practicing now!
            </p>
            <div className="h-1 w-16 bg-gradient-to-r from-green-500 to-blue-500 mx-auto mt-4 rounded-full"></div>
          </div>
          <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {[...freeMocks, ...purchasedPaidMocks].map(renderMockCard)}
          </div>
        </section>
      )}

      {/* Section 2: Paid Individual Mocks */}
      {paidMocks.length > 0 && (
        <section>
          <div className="mb-8">
            <h2 className="text-3xl font-bold text-center mb-2 bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent">
              💎 Premium Mock Tests
            </h2>
            <p className="text-center text-muted-foreground">
              Individual premium mock tests with detailed analytics and solutions
            </p>
            <div className="h-1 w-16 bg-gradient-to-r from-amber-500 to-orange-500 mx-auto mt-4 rounded-full"></div>
          </div>
          <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {paidMocks.map(renderMockCard)}
          </div>
        </section>
      )}

      {/* Section 3: Mock Bundles */}
      <section>
        {/* <div className="mb-8">
          ...
          <p className="text-center text-muted-foreground">
            Get the best value with our comprehensive mock test bundles
          </p>
          <div className="h-1 w-16 bg-gradient-to-r from-purple-500 to-pink-500 mx-auto mt-4 rounded-full"></div>
        </div> */}
        <MockBundlesList
          bundles={bundles}
          userMockSubscriptions={purchasedMockIds}
          userBundleSubscriptions={purchasedBundleIds}
          clerkUserId={userId}
        />
      </section>
    </div>
