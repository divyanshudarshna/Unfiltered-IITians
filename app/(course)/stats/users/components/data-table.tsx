...
import { ChevronDown, Search, Download, Filter, Users, Crown, CreditCard } from "lucide-react"
...
export function DataTable({
  ...
  const stats = React.useMemo(() => {
    ...
    const avgMockScore = data.length > 0 
      ? data.reduce((sum, user) => sum + (user.avgMockScore || 0), 0) / data.length 
      : 0
  ...
  const exportData = () => {
    ...
    const csvContent = [
      ...
      ...exportData.map(user => [
        ...
        user.mockAttemptsCount?.toString() || '0',
        user.avgMockScore?.toString() || '0',
    ...
    <div className="w-full max-w-none space-y-4">
      ...
      <Card className="w-full border-0 shadow-lg bg-card">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="w-full rounded-lg border border-gray-200 dark:border-gray-700 overflow-x-auto">
